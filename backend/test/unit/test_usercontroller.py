...
@pytest.mark.parametrize('daoList, expected',
    [  
        ([{"email": 'gloria@gloria.se'}], {"email": 'gloria@gloria.se'}),
        ([{"email": 'gloria@gloria.se'}, {"email": 'gloria@gloria.se'}, {"email": 'gloria@gloria.se'}], {"email": 'gloria@gloria.se'}),
        ([], None)
    ]
)
def test_valid_email(sut, expected):
    ValidEmailResult = sut.get_user_by_email(email="gloria@gloria.se")
...
def test_UserController_multipleUsers(capsys):
    ...
    mockedDAO.find.return_value = [{"email": 'gloria@gloria.se'}, {"email": 'gloria@gloria.se'}, {"email": 'gloria@gloria.se'}]

    mockedUser = UserController(dao=mockedDAO)

    mockedUser.get_user_by_email(email='gloria@gloria.se')
    captured_print = capsys.readouterr()
    assert captured_print.out == 'Error: more than one user found with mail gloria@gloria.se\n'
